...
class GeneralManagementRecord(TimestampedModel):
    ...
    client = models.ForeignKey(
        ...
        related_name="general_management_records",
    )

    client_text = models.CharField(
        max_length=180,
        blank=True,
        verbose_name="Cliente libre",
    )

    detail_name = models.CharField(
        max_length=200,
        blank=True,
        verbose_name="Nombre / Detalle",
    )

    service_type = models.CharField(
        max_length=30,
        choices=GeneralServiceType.choices,
    ...
    status = models.CharField(
        max_length=30,
        choices=GeneralPaymentStatus.choices,
        default=GeneralPaymentStatus.CEO_PAID,
        verbose_name="Estado",
    )

    receipt_drive_url = models.URLField(
        blank=True,
        verbose_name="Link Recibo Drive",
    )
    ...
    created_by = models.ForeignKey(
        ...
        blank=True,
        on_delete=models.SET_NULL,
        related_name="created_general_management_records",
    )
...
class CredentialPurchase(TimestampedModel):
    client = models.ForeignKey(
        ...
        related_name="credential_purchases",
    )
    ...
    credential_name = models.CharField(
        max_length=180,
        verbose_name="Credencial / recurso",
    )
    ...
    account_email = models.EmailField(
        blank=True,
        help_text=(
            "Correo/usuario de referencia. "
            "No se guardan contraseñas."
        ),
    )

    secure_reference_url = models.URLField(
        blank=True,
        help_text=(
            "Link seguro/Drive donde el equipo autorizado "
    ...
    created_by = models.ForeignKey(
        ...
        blank=True,
        on_delete=models.SET_NULL,
        related_name="created_credential_purchases",
    )
...
class DomainHostingRecord(TimestampedModel):
    client = models.ForeignKey(
        "clients.Client",
        on_delete=models.CASCADE,
        related_name="domain_hosting_records",
    )
    ...
    operation_type = models.CharField(
        max_length=20,
        choices=DomainOperationType.choices,
        default=DomainOperationType.NEW,
        verbose_name="Tipo",
    )
    ...
    domain_expiry_date = models.DateField(
        null=True,
        blank=True,
        verbose_name="Fecha de caducidad del dominio",
    )

    domain_status = models.CharField(
        max_length=24,
        choices=DomainRecordStatus.choices,
        default=DomainRecordStatus.PENDING_PURCHASE,
        verbose_name="Estado del dominio",
    )

    domain_cost = models.DecimalField(
        max_digits=12,
        decimal_places=2,
        default=Decimal("0.00"),
        verbose_name="Costo del dominio",
    )

    domain_receipt_drive_url = models.URLField(
        blank=True,
        default="",
        verbose_name="Recibo del dominio · Google Drive",
    )

    server_provider = models.CharField(
        max_length=30,
        choices=HostingProvider.choices,
        default=HostingProvider.SITEGROUND,
        verbose_name="Proveedor servidor / hosting",
    )

    server_provider_other = models.CharField(
        max_length=120,
        blank=True,
        verbose_name="Otro proveedor",
    )
    ...
    server_cost = models.DecimalField(
        ...
        decimal_places=2,
        default=Decimal("0.00"),
        verbose_name="Costo servidor / hosting",
    )

    server_receipt_drive_url = models.URLField(
        blank=True,
        default="",
        verbose_name="Recibo del hosting · Google Drive",
    )
    ...
    receipt_drive_url = models.URLField(
        blank=True,
        verbose_name="Recibo Drive (legado)",
    )
    ...
    created_by = models.ForeignKey(
        ...
        blank=True,
        on_delete=models.SET_NULL,
        related_name="created_domain_hosting_records",
    )
    ...
    updated_by = models.ForeignKey(
        ...
        blank=True,
        on_delete=models.SET_NULL,
        related_name="updated_domain_hosting_records",
    )
    ...
    def provider_label(self):
        if (
            ...
            and self.server_provider_other.strip()
        ):
            return (
                self.server_provider_other.strip()
            )

        return (
            self.get_server_provider_display()
        )
    ...
    def project_domain_label(self):
        if (
            self.operation_type
            == DomainOperationType.MIGRATION
        ):
            if self.domain_status in {
                DomainRecordStatus.MIGRATED,
                DomainRecordStatus.LEGACY_READY,
                DomainRecordStatus.PURCHASED,
            }:
        ...
        mapping = {
            DomainRecordStatus.PENDING_PURCHASE:
                "Pendiente de compra",

            DomainRecordStatus.PURCHASED:
                "Comprado y registrado",

            DomainRecordStatus.LEGACY_PENDING:
                "Pendiente de compra",
        ...
        return mapping.get(
            self.domain_status,
            self.get_domain_status_display(),
        )
    ...
    def allowed_statuses(self):
        if (
            ...
            return [
                DomainRecordStatus.PENDING_MIGRATION,
                DomainRecordStatus.MIGRATED,
            ]
    ...
    def clean(self):
        ...
        if (
            self.server_provider
            == HostingProvider.OTHER
            and not self.server_provider_other.strip()
        ):
            errors["server_provider_other"] = (
        ...
        if (
            ...
            errors["domain_status"] = (
                "El estado no corresponde "
    ...
    def save(self, *args, **kwargs):
        ...
        if (
            self.receipt_drive_url
            and not self.server_receipt_drive_url
        ):
            self.server_receipt_drive_url = (
                self.receipt_drive_url
            )
    ...
    def __str__(self):
        return (
            f"{self.client.business_name} · "
            f"{self.domain_name or self.get_operation_type_display()}"
        )
...
class CredentialType(models.TextChoices):
    ...
    WEBMAIL = (
        "webmail",
        "Webmail / correo corporativo (legado)",
    )
    ...
    GOOGLE_LSA = (
        "google_lsa",
        "Google LSA / Guarantee (legado)",
    )

    SEARCH_CONSOLE = (
        "search_console",
        "Google Search Console (legado)",
    )

    ANALYTICS = (
        "analytics",
        "Google Analytics (legado)",
    )
...
class ProjectCredential(TimestampedModel):
    client = models.ForeignKey(
        "clients.Client",
        on_delete=models.CASCADE,
        related_name="project_credentials",
    )
    ...
    custom_name = models.CharField(
        max_length=120,
        blank=True,
        verbose_name="Nombre personalizado / proveedor",
    )
    ...
    visible_to_team = models.BooleanField(
        default=False,
        verbose_name="Visible en resumen para las áreas",
    )
    ...
    created_by = models.ForeignKey(
        ...
        blank=True,
        on_delete=models.SET_NULL,
        related_name="created_project_credentials",
    )
    ...
    updated_by = models.ForeignKey(
        ...
        blank=True,
        on_delete=models.SET_NULL,
        related_name="updated_project_credentials",
    )
    ...
    def has_access_data(self):
        return bool(
            self.account_email.strip()
            or self.password_encrypted
            or self.login_url
        )
    ...
    def clean(self):
        ...
        if (
            self.credential_type
            == CredentialType.OTHER
            and not self.custom_name.strip()
        ):
            errors["custom_name"] = (
...
class ProductionRecord(TimestampedModel):
    ...
    project_site = models.CharField(
        max_length=240,
        verbose_name="Proyecto/Sitio",
    )

    plan_start = models.DateField(
        null=True,
        blank=True,
        verbose_name="Inicio plan",
    )

    design_status = models.CharField(
        max_length=20,
        choices=DesignProductionStatus.choices,
        default=DesignProductionStatus.INCOMPLETE,
        verbose_name="DISEÑO",
    )

    tech_status = models.CharField(
        max_length=20,
        choices=CompleteStatus.choices,
        default=CompleteStatus.INCOMPLETE,
        verbose_name="ESTADO TEC",
    )

    status = models.CharField(
        max_length=30,
        choices=ProductionStatus.choices,
        default=ProductionStatus.START,
        verbose_name="Estado",
    )
...
# ---------------------------------------------------------------------------
# FICHA DE PRODUCCIÓN WEB · DESARROLLO
# ---------------------------------------------------------------------------
...
class WebProductionSheet(TimestampedModel):
    project = models.OneToOneField(
        "projects.Project",
        on_delete=models.CASCADE,
        related_name="web_production_sheet",
    )

    main_page_target = models.PositiveIntegerField(
        default=8,
        verbose_name="Cantidad páginas principales",
    )

    county_target = models.PositiveIntegerField(
        default=0,
        verbose_name="Cantidad de condados",
    )

    services_per_county_target = (
        models.PositiveIntegerField(
            default=0,
            verbose_name="Servicios por condado",
        )
    )
    ...
    updated_by = models.ForeignKey(
        ...
        blank=True,
        on_delete=models.SET_NULL,
        related_name="updated_web_production_sheets",
    )
...
class WebProductionPage(ProductionSeoBase):
    ...
    def __str__(self):
        return (
            self.name
            or f"Página {self.pk or ''}"
        )


class WebProductionCounty(
    ProductionSeoBase
):
    sheet = models.ForeignKey(
        WebProductionSheet,
    ...
    def __str__(self):
        return (
            self.name
            or f"Condado {self.pk or ''}"
        )


class WebProductionCountyService(
    ProductionSeoBase
):
    ...
    def __str__(self):
        return (
            f"{self.county} · "
            f"{self.name or self.service_name or 'Servicio'}"
        )


class WebProductionCity(
    ProductionSeoBase
):
    sheet = models.ForeignKey(
        WebProductionSheet,
    ...
    class Meta:
        ...
        verbose_name_plural = (
            "Ciudades de producción web"
        )

    def __str__(self):
        return (
            self.name
            or f"Ciudad {self.pk or ''}"
        )
