...
def client_list(request):
    ...
    if q:
        clients = clients.filter(Q(business_name__icontains=q) | Q(first_name__icontains=q) | Q(last_name__icontains=q) | Q(phone__icontains=q))
    return render(request, "clients/list.html", {"clients": clients, "q": q})


@login_required
def client_detail(request, pk):
    client = get_object_or_404(Client.objects.prefetch_related("projects", "purchased_plans", "domains"), pk=pk)
...
def client_create(request):
    ...
    return render(request, "shared/form.html", {"form": form, "title": "Nuevo cliente", "subtitle": "Registro administrativo del cliente."})
